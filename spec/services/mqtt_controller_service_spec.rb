  ...
  shared_examples_for "options keys check" do |key|
    context "when options key is missing" do
      ...
      it "raise exception" do
        expect { subject }.to raise_error("Options must have task_id, task_url, mqtt_client_url and mqtt_client_guid keys")
      end
    end
  end

  ["task_id", "task_url", "mqtt_client_url", "mqtt_client_guid"].each do |key|
